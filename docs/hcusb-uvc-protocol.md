...
### 5.9 USB_BODYTEMP_COMPENSATION（2044/2045，组 3 sub 8）— 18 字节，真机验证

GET 响应示例：`01 01 02 ee 03 00 00 e8 03 00 00 fa 04 00 00 01 02 02`

| 偏移 | 类型 | 字段 |
|---|---|---|
| 0 | u8 | GET 回显 0x01；SET 时写 byChannelID=1 |
| 1 | u8 | **byEnabled 体温补偿使能**（SET 同位置写 0/1） |
| 2 | u8 | 0x02（模式/类型，未深究） |
| 3..15 | 3×u32 | 设备实时计算值（观测 1006/1000/1271±3，随时间漂移），写入会被覆盖 |
| 15..18 | 3×u8 | 0x01 0x02 0x02 |

**补偿效果（A/B/A 实测，采样间隔秒级，排除目标降温干扰）**：

| | 面部最高 | 面部平均 | 47°C 热水杯 |
|---|---|---|---|
| 补偿 ON | 37.8 °C | 33.7 °C | 47.1 °C |
| 补偿 OFF | 36.5 °C | 30.8 °C | 46.3 °C |

即补偿把**皮肤温度段整体抬高约 3°C**（映射为估算核心体温），对高温目标
基本无影响——典型体温筛查曲线。关闭后温度矩阵与 OSD 均为接近真实的表面
温度（两者仍然一致）。

## 6. 实测备注

- 发任何命令前确认视频流已启动，否则 selector 5 的 SET_CUR 会超时。
- **重插/重 attach 后必须先做版本探测**（GET_LEN+GET_CUR sel4，见 §2），
  否则命令通道锁定：sel5 SET 被静默接受、GET_LEN 恒返回 0。
- 命令出错后读 selector 6 拿错误码。
- GetDataLen 返回上限 0x200；真实长度以 GET_LEN 读到的 u32 为准。
- 所有多字节整数均小端。
- **开流有旧帧缓冲**：start_stream 后设备突发吐出约 5–7 帧（约 0.2 秒）
  关机前积压的旧数据，表现为头几帧"平均 fps"虚高（>100）；突发结束后
  才是 30fps 实时帧。要拍"当下"画面请跳过开流后的前 ~10 帧。
- **FFC（平场校正）**：非制冷微测辐射热计需要周期性用机械快门校正
  （间歇的"咔"声），校正瞬间画面冻结/跳变 1–2 帧（约 100–300ms）。
  连续记录时可通过帧停顿/画面均值突变识别并剔除这几帧。
